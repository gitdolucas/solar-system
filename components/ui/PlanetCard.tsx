    ...
    <div className="flex flex-col gap-5">
      <div>
        <div className="flex items-center gap-3 mb-2">
          <div className="w-4 h-4 rounded-full flex-shrink-0" style={{ background: planet.cor }} />
        ...
        <h1
          ...
          style={{ fontFamily: 'var(--font-pacifico)', color: planet.cor }}
        >
          {planet.nome}
        </h1>
      </div>
      ...
      <div className="flex flex-col gap-2">
        <StatRow icon="☀️" label="Distância do Sol" value={planet.distanciaDaSol} />
        <StatRow icon="🌍" label="Tamanho" value={planet.tamanhoRelativo} />
        <StatRow icon="🌡️" label="Temperatura" value={planet.temperatura} />
      </div>
          ...
          {planet.curiosidades.map((c, i) => (
            ...
            >
              <span
                ...
                style={{ background: planet.cor + '33', color: planet.cor, fontFamily: 'var(--font-ubuntu)' }}
              >
                {i + 1}
              </span>
      ...
      {planet.moons.length > 0 && (
        <div
          ...
          style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)' }}
        >
          <p className="text-sm" style={{ color: '#8fa3b8', fontFamily: 'var(--font-ubuntu)' }}>
            🌙 <strong style={{ color: 'white' }}>{planet.moons.length} lua{planet.moons.length > 1 ? 's' : ''}</strong> visíveis ao redor do planeta
          </p>
        </div>
      )}
    </div>
